...
const Bookmark = (props) => {
    ...
    const { user, onBookmarkAddReq, onBookmarkFetchReq } = props;
    ...
    const handleOnBookmarkAdd = async (name, url, description) => {
      setOpenAddBookmark(false);
      if(onBookmarkAddReq===undefined) return;

      const bookmarkAdd = async (name, url, description) => {
    ...
    return(
        <div style={{ display: "flex", paddingLeft: 15 }}>
            ...
            <BookmarkAdd open={ openAddBookmark } updateDialogStateClose={ handleOnBookmarkAdd }/>
        </div>
    );
}
